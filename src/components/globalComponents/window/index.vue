<template>
  ...
  >
    ...
    >
      <div class="window-bars">
        <div
          ...
          :class="[
            item.className,
            index === 1 && page_config.isFullScreen ? 'no-minimize' : '',
          ]"
        >
    ...
    <div
      v-for="(stick, index) in page_config.sticks"
      :key="index"
      class="vdr-stick"
...
const isBarShow = computed(() => {
  ...
  let status =
    isFullScreen === false ||
    (isFullScreen === true && cursorPointerY === true);
  // 只有当前顶层app才能去对全局状态进行变化 barTop只需要判断全屏和是否到顶部
  if (isScreenFacade.value) {
    systemStore[SET_FULL_SCREENBAR](
      isFullScreen === true && cursorPointerY === true
    );
  }
  return status;
...
onMounted(() => {
  ...
  initWindowStatus(windom, props.width, props.height);
  page_config.shows = props.show;
  // web页面移动
  domEvents.set("mousemove", (ev) => {
    emit("eventMousemove", ev);
    const {
      winBarConfig: { winBarStart },
      sticksConfig: { sticksStart },
    } = page_config;
    const { clientY } = ev;
