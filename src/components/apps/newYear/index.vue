...
function render() {
  ...
  if (limiterTick >= limiterTotal) {
    if (isMousedown) {
      fireworks.push(
        new Firework(canvasWidth / 2, canvasHeight, mouseCoord.x, mouseCoord.y)
      );
      limiterTick = 0; // 清0
    }
  } else {
  ...
  for (let x = 0; x < canvasWidth; x += 4) {
    for (let y = 0; y < canvasHeight; y += 4) {
      ...
      if (imageData[pxAlphaIndex] > 0) {
        particles.push(new TextParticle({ x, y, speed: 10 }));
      }
    }
  }
  return particles;
